...
public class YuvToRgbConverter {
    public  static void yuvToRgb(Context context, Image image, Bitmap output){
        ...
        ScriptIntrinsicYuvToRGB scriptIntrinsicYuvToRGB = ScriptIntrinsicYuvToRGB.create(rs, Element.U8_4(rs));
        ...
        byte[] yubBuffer = new byte[pixelCount * pixelSizeBits/ 8];
        imageToByteArray(image,yubBuffer,pixelCount);
        Type elemType = new Type.Builder(rs, Element.YUV(rs)).setYuvFormat(ImageFormat.NV21).create();
        Allocation inputAllocation = Allocation.createSized(rs, elemType.getElement(), yubBuffer.length);
        Allocation outputAllocation = Allocation.createFromBitmap(rs, output);

        inputAllocation.copyFrom(yubBuffer);
    ...
    private static void imageToByteArray(Image image, byte[] outputBuffer, int pixelCount){
        ...
        for(int planeIndex = 0; planeIndex < imagePlanes.length ; planeIndex++){
            ...
            Rect planeCrop;
            if(planeIndex == 0) planeCrop = imageCrop;
            else{
            ...
            int rowLength;
            if(pixelStride == 1 && outputStride == 1) rowLength = planeWidth;
            else {
